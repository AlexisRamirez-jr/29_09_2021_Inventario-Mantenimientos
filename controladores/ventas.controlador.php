...
class ControladorVentas{
	...
	static public function ctrCrearVehiculos(){
		...
		if(isset($_POST["nuevoMatricula"])){
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoMatricula"]) &&
			preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoTipo"]) &&
			   preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoModelo"]) ) {
			   	...
			   	$datos = array("matricula"=>$_POST["nuevoMatricula"],
					           ...
					           "anio"=>$_POST["nuevoAño"],
					           "precioadquisicion"=>$_POST["nuevoPrecio"],
							   "fechaadquirido"=>$_POST["nuevaFechaCompra"] );

			   	$respuesta = ModeloVentas::mdlIngresarVehiculos($tabla, $datos);
...
/*=============================================
		...
		if(isset($_POST["editarMatricula"])){
			...
			preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarTipo"]) &&
			   ...
			   preg_match('/^[0-9]+$/', $_POST["editarAño"]) 
				   ...
				   "anio"=>$_POST["editarAño"],
				   "precioadquisicion"=>$_POST["editarPrecio"],
				   "fechaadquirido"=>$_POST["editarFecha"] );

			   	$respuesta = ModeloVentas::mdlEditarVehiculos($tabla, $datos);
	...
	static public function ctrCrearVenta(){
		...
		//	$totalProductosComprados = array();
			...
			//    $orden = "id";
			    ...
			    $traerProducto = ModeloVentas::mdlMostrarVentas($tablaProductos, $item, $valor);
	...
	static public function ctrEditarVenta(){
		...
		if(isset($_POST["editarVenta"])){
			...
			if($_POST["listaProductos"] == ""){
				...
				$listaProductos = $traerVenta["matricula"];
			...
			if($cambioProducto){
				...
				$productos =  json_decode($traerVenta["matricula"], true);

				$totalProductosComprados = array();

				foreach ($productos as $key => $value) {

					array_push($totalProductosComprados, $value["id"]);
					
					$tablaProductos = "mantenimientos";
				...
				foreach ($listaProductos_2 as $key => $value) {
					...
					$tablaProductos_2 = "mantenimientos";
	...
	static public function ctrEliminarVenta(){
		...
		if(isset($_GET["idVenta"])){
			...
			if($respuesta == "ok"){
				...
				swal({
					  type: "success",
					  title: "Registro ha sido borrado correctamente",
					  showConfirmButton: true,
					  confirmButtonText: "Cerrar"
					  }).then(function(result){
								if (result.value) {
	...
	public function ctrDescargarReporte(){
		...
		if(isset($_GET["reporteflota"])){
			...
			echo utf8_decode("<table border='0'> 
					...
					<td style='font-weight:bold; border:1px solid #eee;'>FECHA_COMPRA</td>
					<td style='font-weight:bold; border:1px solid #eee;'>FECHA_REGISTRO</td>		
			...
			//	$vendedor = ControladorUsuarios::ctrMostrarUsuarios("id", $item["id_vendedor"]);
			 ...
			 echo utf8_decode("<tr>
			 			<td style='border:1px solid #eee;'>".$item["matricula"]."</td> 
			 			<td style='border:1px solid #eee;'>".$item["tipo"]."</td>
			 			<td style='border:1px solid #eee;'>".$item["modelo"]."</td>
						<td style='border:1px solid #eee;'> ".$item["anio"]. "</td>
						<td style='border:1px solid #eee;'>$ ".number_format($item["precioadquisicion"],2)."</td>
						<td style='border:1px solid #eee;'>".substr($item["fechaadquirido"],0,10)."</td>
						<td style='border:1px solid #eee;'>".substr($item["fecharegistro"],0,10)."</td>
	...
	public function ctrSumaTotalVentas(){
		...
		$respuesta = ModeloVentas::mdlSumaTotalEmpleados($tabla);
...
*/
	...
	/*=============================================
	SUMA TOTAL INVERSION
	=============================================

	public function ctrSumaTotalInversion(){

		$tabla = "vehiculos";

		$respuesta = ModeloVentas::mdlSumaTotalInversion($tabla);
...
*/
	...
	/*=============================================
	SUMA TOTAL MAntenimientos
	=============================================

	public function ctrSumaTotalMantenimientos(){

		$tabla = "mantenimientos";

		$respuesta = ModeloVentas::mdlSumaTotalMantenimientos($tabla);
	...
	*/
	...
	static public function ctrDescargarXML(){
		...
		//	$listaProductos = json_decode($ventas["matricula"], true);
	...
	//	$valor = $ventas["id_cliente"];
		...
		$objetoXML->startElement("xml");// Inicio del nodo raíz

			//$objetoXML->writeAttribute("", ""); // Atributo etiqueta principal
			$objetoXML->writeAttribute("informe", "vehiculo");

				$objetoXML->startElement("id");// Inicio del nodo hijo
				$objetoXML->writeAttribute("CODIGO", $ventas["id"]);
				$objetoXML->endElement(); // Final del nodo hijo
			
				$objetoXML->startElement("mt");// Inicio del nodo hijo
				$objetoXML->writeAttribute("MATRICULA", $ventas["matricula"]);
				$objetoXML->endElement(); 

				$objetoXML->startElement("tp");// Inicio del nodo hijo
				$objetoXML->writeAttribute("TIPO", $ventas["tipo"]);
				$objetoXML->endElement(); 

				$objetoXML->startElement("md");// Inicio del nodo hijo
				$objetoXML->writeAttribute("MODELO", $ventas["modelo"]);
				$objetoXML->endElement(); 

				$objetoXML->startElement("an");// Inicio del nodo hijo
				$objetoXML->writeAttribute("AÑO", $ventas["anio"]);
				$objetoXML->endElement(); 

				$objetoXML->startElement("p");// Inicio del nodo hijo
				$objetoXML->writeAttribute("PRECIO", $ventas["precioadquisicion"]);
				$objetoXML->endElement(); 

				$objetoXML->startElement("fa");// Inicio del nodo hijo
				$objetoXML->writeAttribute("FECHACOMPRA", $ventas["fechaadquirido"]);
				$objetoXML->endElement(); 
		...
		/*	$objetoXML->writeRaw('<fe:Invoice xmlns:fe="http://www.dian.gov.co/contratos/facturaelectronica/v1" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:clm54217="urn:un:unece:uncefact:codelist:specification:54217:2001" xmlns:clm66411="urn:un:unece:uncefact:codelist:specification:66411:2001" xmlns:clmIANAMIMEMediaType="urn:un:unece:uncefact:codelist:specification:IANAMIMEMediaType:2003" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:qdt="urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2" xmlns:sts="http://www.dian.gov.co/contratos/facturaelectronica/v1/Structures" xmlns:udt="urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.dian.gov.co/contratos/facturaelectronica/v1 ../xsd/DIAN_UBL.xsd urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2 ../../ubl2/common/UnqualifiedDataTypeSchemaModule-2.0.xsd urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2 ../../ubl2/common/UBL-QualifiedDatatypes-2.0.xsd">');
			...
			//foreach ($listaProductos as $key => $value) {
				...
				$objetoXML->text($ventas["id"].", ");
				$objetoXML->text($ventas["matricula"].", ");
				$objetoXML->text($ventas["tipo"].", ");
				$objetoXML->text($ventas["modelo"].", ");
				$objetoXML->text($ventas["anio"].", ");
				$objetoXML->text($ventas["precioadquisicion"].", ");
				$objetoXML->text($ventas["fechaadquirido"].", ");
				$objetoXML->text($ventas["fecharegistro"].", ");
